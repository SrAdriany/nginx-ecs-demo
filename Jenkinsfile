            ...
            steps {
                script {
                    withAWS(credentials: 'aws-lab', region: params.AWS_REGION) {
                        ...
                        def filteredDef = sh(
                             script: """echo '${taskDef}' | jq '.taskDefinition | {family, containerDefinitions, executionRoleArn, taskRoleArn, networkMode, volumes, placementConstraints, requiresCompatibilities, cpu, memory}' | jq 'with_entries(select(.value != null))'""",
                            returnStdout: true
                        ).trim()
